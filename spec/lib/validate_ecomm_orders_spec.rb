...
describe ValidateEcommOrders do
    ...
    context 'methods' do
        it "validates a single order manually" do
          ...
          expect( ValidateEcommOrders.manual_check( qaorder, 'y' ) ).to eq 0
        end

        it "invalidates a single order manually" do
          my_order = '1'
          CreateEcommOrders.manual_add( qaorder, my_order )
          expect( ValidateEcommOrders.manual_check( qaorder, 'n' ) ).to eq 1
        end
        ...
        it "validates a single new order via a DB query" do
          qaorder.config_list = [ CONFIGFILE_BASIC ]
          expect( CreateEcommOrders.api_add( qaorder ) ).to eq 0
          expect( ValidateEcommOrders.db_check_new_all( qaorder ) ).to eq 0
        end

        it "invalidates an incorrect new order via a DB query" do
          my_invalid_order = '1'
          CreateEcommOrders.manual_add( qaorder, my_invalid_order )
          expect( ValidateEcommOrders.db_check_new_all( qaorder ) ).to_not eq 0
        end
        ...
        it "invalidates an incorrect booked order via a DB query" do
          my_invalid_order = '1'
          CreateEcommOrders.manual_add( qaorder, my_invalid_order )
          expect( ValidateEcommOrders.db_check_booked_all( qaorder ) ).to_not eq 0
        end
        ...
        it "validates a booked order via a DB query" do
          ...
          expect( ValidateEcommOrders.db_check_booked_all( qaorder ) ).to eq 0
        end

        it "invalidates an incorrect invoiced order via a DB query" do
          my_invalid_order = '1'
          CreateEcommOrders.manual_add( qaorder, my_invalid_order )
          expect( ValidateEcommOrders.db_check_invoiced_all( qaorder ) ).to_not eq 0
        end

        it "invalidates an incorrect canceled order via a DB query" do
          my_invalid_order = '1'
          CreateEcommOrders.manual_add( qaorder, my_invalid_order )
          expect( ValidateEcommOrders.db_check_canceled_all( qaorder ) ).to_not eq 0
        end

        it "invalidates an incorrect gift certificate order via a DB query" do
          my_invalid_order = '1'
          CreateEcommOrders.manual_add( qaorder, my_invalid_order )
          expect( ValidateEcommOrders.db_check_giftcertificate_all( qaorder ) ).to_not eq 0
        end
    end
end
